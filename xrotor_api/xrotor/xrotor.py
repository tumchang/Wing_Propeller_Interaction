...
class XRotor(object):
    ...
    def max_iter(self, value):
        ...
        result = self._lib.set_max_iter(byref(c_int(100)))
        print(result)
    ...
    def station_conditions(self):
        ...
        vt = np.zeros(n, dtype=c_float, order='F')
        va = np.zeros(n, dtype=c_float, order='F')
        vt_slip = np.zeros(n, dtype=c_float, order='F')
        va_slip = np.zeros(n, dtype=c_float, order='F')

        self._lib.get_station_conditions(byref(c_int(n)),
                                         xi.ctypes.data_as(fptr),
                                         re.ctypes.data_as(fptr),
                                         ma.ctypes.data_as(fptr),
                                         cl.ctypes.data_as(fptr),
                                         cd.ctypes.data_as(fptr),
                                         cm.ctypes.data_as(fptr),
                                         vt.ctypes.data_as(fptr),
                                         va.ctypes.data_as(fptr),
                                         vt_slip.ctypes.data_as(fptr),
                                         va_slip.ctypes.data_as(fptr)
                                         )

        return xi, re, ma, cl, cd, cm, vt, va,  vt_slip, va_slip
    ...
    def save_propeller_geometry(self):
        self._lib.save_prop()
